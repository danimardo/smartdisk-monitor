...
- [X] T020 Implementar el planificador de recopilación con sus frecuencias configurables y la reducción en batería en `src-tauri/src/collectors/planificador.rs` (FR-030). Bucle real en un hilo bloqueante (`commands::iniciar_planificador`, `tauri::async_runtime::spawn_blocking`, cero dependencias nuevas), que sondea cada 1 s (`open-questions.md` J.34) y decide con `trabajos_debidos` (función pura, probada con tiempo inyectado). Batería real vía `platform::energia::en_bateria()` (FFI `GetSystemPowerStatus`, mismo patrón que `platform::locale`). `refresh_smart` se separó en SMART y rendimiento (`open-questions.md` J.38) para que cada uno respete su propia cadencia. Modelo de hilos documentado en ADR-033
- [X] T021 Implementar la emisión de eventos hacia la interfaz, sin que ninguna pantalla necesite sondear (ADR-015). Los cuatro eventos que faltaban (`metrics:updated`, `inventory:changed`, `source:degraded`, `monitoring:paused`/`resumed`) ya se emiten, junto con `alerts:changed` que ya existía. `SourceHealth` pasa de `status: String` sin tipar a un `SourceStatus` real, construido por `actualizar_source_health` (`open-questions.md` J.37: agregado por tipo de colector, ok/timeout/error esta primera vez). `inventory:changed.updated` se deja vacío a propósito (J.35). `metrics:updated.historyWriteHalted` refleja un cálculo real de espacio libre, sin detener todavía ninguna escritura (J.40, seguimiento explícito)
- [X] T022 Reanudar siempre la monitorización al arrancar, aunque quedara pausada, en `src-tauri/src/lib.rs` (FR-031). Satisfecho por construcción: `AppState.paused` nunca se persistió (ya lo documentaba el propio código), así que todo arranque del proceso empieza activo en cuanto el bucle en segundo plano existe para leerlo — no hizo falta ninguna lógica de "reanudar", solo no volver a pausar desde disco. Apagado limpio del bucle vía `RunEvent::Exit`/`ExitRequested` en `lib.rs`
...
- [X] T087 [P] [US6] Implementar la exportación tabular y estructurada en `src-tauri/src/reporting/export.rs`. Ningún documento fijaba el contenido exacto: registrado en `docs/open-questions.md` J.30 antes de programar — CSV/JSON son el volcado completo (una fila por dispositivo/métrica/marca de tiempo, con la misma cascada de resolución que las gráficas), HTML es un resumen legible aparte. `schemaVersion` en columna, dato ausente como `"N/A"`/`null`, nunca cero
- [X] T088 [P] [US6] Implementar el informe imprimible en `src-tauri/src/reporting/informe.rs`: autónomo (CSS embebido, sin recursos remotos, tema claro forzado, hoja de impresión propia), identidad y alertas del intervalo por dispositivo. Las alertas muestran `ruleKey` tal cual, no una frase (ADR-030: el backend no traduce, y este HTML no tiene acceso a `$lib/i18n`)
- [X] T089 🔴 test-first [US6] Escribir las pruebas de anonimización en `src-tauri/src/reporting/anonimizar.rs` (inline, no un fichero `_test.rs` aparte — mismo criterio que T077): sin números de serie, sin nombre de equipo, sin rutas de usuario, sustitución consistente y cadena más larga primero. 8 pruebas, escritas antes que el motor de sustitución
- [X] T090 [US6] Implementar el paquete de diagnóstico anonimizado por defecto en `src-tauri/src/reporting/diagnostico.rs`, incluyendo el registro de actividad (FR-029c). `smart_snapshots.raw_json_path` no tiene contenido real que leer (ningún colector lo escribe, T036): el SMART bruto se vuelve a consultar en vivo con `smartctl` al generar el paquete (`docs/open-questions.md` J.31). ZIP real vía el crate `zip` (ADR-032, autorizado explícitamente por el usuario frente a un escritor propio)
- [X] T091 [US6] Conectar `export_report`, `preview_diagnostic_zip` y `create_diagnostic_zip` en `src-tauri/src/commands/mod.rs`. El destino de guardado lo elige el usuario con `tauri-plugin-dialog` (ADR-031, autorizado explícitamente frente a una carpeta fija): sin ese permiso no había ninguna vía legítima para que la interfaz propusiera dónde guardar sin abrir una segunda vía de acceso al sistema de ficheros (principio IX)
- [X] T092 [US6] Implementar la pantalla de informes en `src/routes/reports/+page.svelte` (US-050, pendiente de diseño: pedir revisión visual). Sin boceto aprobado a diferencia de pruebas: se preguntó al usuario cómo abordarlo (`AskUserQuestion`) y se autorizó construirla con el catálogo actual según `docs/ui-design.md` Apéndice C — intervalo (`SegmentedControl` + `DateRangePicker`), discos incluidos (`Switch` por disco), tres tarjetas de formato con destino elegido por diálogo nativo (`chooseSavePath`, ADR-031), y el paquete de diagnóstico con vista previa inline (entradas, tamaño total, campos sustituidos) antes de guardar (US-051)
- [X] T093 [P] [US6] Añadir las claves de informes y la advertencia de contenido identificativo a los dos diccionarios de `src/lib/i18n/`: 38 claves nuevas (225 → 263), incluidas seis claves de error de T083 que se habían quedado sin traducir (`error.volumeNotFound`, `testBusy`, `testUnsupported`, `testInsufficientSpace`, `testIoFailed`, `pathInvalid`)
- [X] T094 [US6] Pasar la definición de terminado de `docs/ui-design.md` §8 en informes: `pnpm check`/`lint`/`verify` en cero, `e2e/ui/reports.spec.ts` nuevo (exportación con destino nativo, vista previa del ZIP, guardado) y `/reports` en `a11y.spec.ts` — sin incumplimientos de axe en ambos temas. Mismo alcance parcial que T086: sin verificar acento no azul ni el recorte exacto en las dos resoluciones mínimas
...
- [X] T095 🔴 test-first [P] [US7] Escribir las pruebas de límites en `src-tauri/src/domain/ajustes.rs` (inline, no un fichero `_test.rs` aparte — mismo criterio que T077/T089): todo valor fuera de rango se rechaza y el anterior se mantiene. `ui-contract.md` nombraba `get_settings`/`set_setting`/`reset_settings` pero nunca escribía la interfaz completa: forma de `Settings` registrada en `docs/open-questions.md` J.32 antes de programar (15 pruebas)
- [X] T096 [US7] Implementar la validación tipada de `settings` en `src-tauri/src/domain/ajustes.rs`. Reutiliza tal cual los límites de frecuencia ya codificados en `collectors::planificador` (D.1, ese módulo ya decía "esto lo hace `domain::ajustes`") y los de capacidad ya decididos en C.1/ADR-019; temperatura y retención llevan límites de edición nuevos, sin medir (J.32)
- [X] T097 [US7] Conectar `set_setting`, `get_settings`, `reset_settings` y `delete_all_data` en `src-tauri/src/commands/mod.rs`. De paso se completó `get_appearance_settings` (T072-adyacente): antes devolvía siempre valores fijos ("system"/`null`/`true`), ahora lee `settings.appearance.*`, que es lo que hacía falta para que el tema e idioma sobrevivan a un reinicio (US-072). Frase de confirmación de `delete_all_data`: el propio nombre de la aplicación, no una palabra ceremonial (no depende de una traducción mantenida en los dos diccionarios)
- [X] T098 [US7] Implementar el cambio de nivel de registro y la apertura de su carpeta, conectados en `src-tauri/src/commands/mod.rs` (no en un fichero `registro.rs` aparte: los comandos de esta sesión viven todos en `commands/mod.rs`, mismo criterio que el resto). `logging::init` se reescribió con `tracing_subscriber::reload` para poder cambiar el nivel en caliente sin reiniciar; `open_log_folder` lanza `explorer.exe` directamente (sin el crate `open`, ya hay precedente de invocar procesos de Windows a mano en este módulo)
- [X] T099 [US7] Implementar el comportamiento al cerrar la ventana, con opción de recordar la decisión, en `src-tauri/src/platform/ventana.rs` (FR-029). Lee `lifecycle.close_action` en cada cierre, no solo al arrancar. **Sin diálogo emergente de primer cierre**: ninguna tarea lo pedía explícitamente y habría exigido un evento y un componente nuevos fuera de alcance; la preferencia se expone en la pantalla de Ajustes (T100) en su lugar, cumpliendo igual US-072 (`docs/open-questions.md` J.19 actualizada)
- [X] T100 [US7] Implementar la pantalla de ajustes en `src/routes/settings/+page.svelte` con `Switch`, `SegmentedControl`, `RadioGroup`, `TextField` y `ConfirmDialog` (pendiente de diseño: pedir revisión visual). Mismo criterio ya autorizado por el usuario para informes (T092): construida con el catálogo actual según `docs/ui-design.md` Apéndice C, sin pedir de nuevo. `Select` no encajaba en ningún grupo: todas las opciones son 2-4 excluyentes cortas, terreno de `SegmentedControl`/`RadioGroup` según la propia guía de esos componentes. Esta pantalla es el primer consumidor real de `theme.set()`/`i18n.set()` (devolvían `{key, value}` desde hace varias historias, a la espera de quien los llamara y persistiera con `set_setting`)
- [X] T101 [P] [US7] Añadir las claves de ajustes a los dos diccionarios de `src/lib/i18n/`: 50 claves nuevas (263 → 313)
- [X] T102 [US7] Pasar la definición de terminado de `docs/ui-design.md` §8 en ajustes: `pnpm check`/`lint`/`verify` en cero, `e2e/ui/settings.spec.ts` nuevo (cambiar una frecuencia, restaurar valores de fábrica de un ámbito, frase de confirmación exacta antes de borrar todo) y `/settings` en `a11y.spec.ts` — sin incumplimientos de axe en ambos temas. Mismo alcance parcial que T086/T094: sin verificar acento no azul ni el recorte exacto en las dos resoluciones mínimas
...
- [X] T103 [US8] Configurar el instalador con WebView2 sin conexión en `src-tauri/tauri.conf.json` (ADR-020; requiere modo plan: es ruta protegida). **Ya estaba configurado** de una sesión anterior a esta (`bundle.windows.webviewInstallMode.type = "offlineInstaller"`, `nsis.installMode = "perMachine"`, idiomas español/inglés): verificado leyendo el fichero, sin necesidad de modo plan porque no hizo falta modificarlo
- [X] T104 [P] [US8] Implementar la pantalla «Acerca de» con versión, licencia y avisos de terceros en `src/routes/settings/+page.svelte` o su ruta propia (US-061). Implementada como diálogo reutilizando `ConfirmDialog` desde el botón `?` ya existente en `Toolbar` (antes sin conectar), no como ruta propia: US-061 pide un diálogo, no una pantalla, y el catálogo no tenía otro componente de superposición. De paso: `get_app_info` devolvía `CARGO_PKG_NAME` (el nombre del *crate*, "smartdisk-monitor"), no el nombre de producto de `tauri.conf.json` — corregido a `app.package_info().name` (que Tauri resuelve desde `productName`), sin lo cual el diálogo habría mostrado el nombre equivocado. Corregidos dos literales de interfaz preexistentes hallados al reutilizar `ConfirmDialog` (`"Cancelar"` y el valor por defecto `"Continuar"` de `confirmLabel`, ninguno pasaba por `t()`) y un fallo real de foco: `Escape` nunca cerraba el diálogo porque el foco no entraba al panel al abrirlo — corregido con un `$effect` que lo mueve, `docs/known-issues.md` #2 actualizada
- [X] T105 [US8] Verificar que la desinstalación **conserva** historial y configuración, y documentar la limpieza manual en `README.md`. Verificado por diseño (ADR-012 ya lo decidía; comprobado que ningún código de esta sesión escribe en el registro de Windows, solo lee el acento) y documentado: nueva sección «Instalación y desinstalación» con la ruta exacta (`%ProgramData%\SmartDisk Monitor\`), qué conserva la desinstalación, la vía normal (Ajustes → Borrar todos los datos, US-073) y la limpieza manual tras desinstalar. **No verificado con una instalación y desinstalación reales**: requiere construir el instalador y ejecutarlo, acción invasiva no realizada sin autorización explícita
- [X] T106 [US8] Verificar las migraciones de `src-tauri/migrations/` desde cada versión publicada anterior, no solo desde cero. No existe todavía ninguna versión publicada (esta es la primera): se verifica el propio mecanismo con una prueba nueva que simula una base que solo llegó a la migración 1 (como dejaría una versión publicada antes de que existiera la 2) y comprueba que `apply_pending` aplica solo la 2, en orden, sin reaplicar la 1, con copia de seguridad previa, y termina en el mismo esquema que una instalación desde cero
- [X] T107 [US8] Comprobar con un monitor de red que la aplicación no emite **ninguna** petición saliente, según el guion del eslabón 10 de `specs/001-monitor-discos-windows/quickstart.md` (SC-014). **Auditoría estática, no medición en vivo** (`docs/open-questions.md` J.33, mismo trato honesto que J.28): sin cliente HTTP entre las dependencias directas ni transitivas reales de Windows (`cargo tree --target x86_64-pc-windows-msvc` no incluye `reqwest`/`hyper`, presentes en `Cargo.lock` solo por una dependencia de `tauri` acotada a Android/iOS); cero `std::net`, `fetch`, `XMLHttpRequest` o `WebSocket` en todo el código; CSP sin ningún origen remoto en `connect-src`. Pendiente la medición real con un instalador construido y un monitor de paquetes, acción invasiva no realizada sin autorización explícita
...
- [ ] T108 [P] Comprobar los mínimos de cobertura: 90 % en `domain/` y `alerts/`, 80 % en el resto de `src-tauri/src/`, 70 % en `src/lib/` sin componentes. **Parcial**: medido con `cargo llvm-cov` y `pnpm test:coverage`. `domain/`+`alerts/` → 94,48 % (supera el 90 %). `src/lib/` sin componentes → 85,74 % de sentencias (supera el 70 %). Resto de `src-tauri/src/` → 72,50 %, por debajo del 80 %; se añadieron pruebas a veinte funciones puras que estaban sin cubrir (`set_setting_impl`, `resolve_target_name`, `nivel_resuelto`, `export_report_impl`, `set_device_alias_impl`, `persist_perf_reading` y otras), pero el hueco restante es arquitectónico, no pereza de pruebas: son los envoltorios `#[tauri::command]` (`State<AppState>`/`AppHandle`, no construibles en un `#[test]`) y los colectores ya aceptados como límite de hardware/SO. Registrado como `docs/open-questions.md` K.6, pendiente de que el usuario elija entre enmendar el umbral o aceptar la excepción documentada
- [X] T109 [P] Verificar que cada componente de `src/lib/components/` tiene prueba `*.browser.test.ts` de sus cinco estados: vacío, cargando, error de fuente, no compatible y dato obsoleto. De los 28 componentes del catálogo, 9 son responsables de al menos uno de los cinco estados y ya tienen `*.browser.test.ts`: `Button` y `DiskCard` (previos) más `EmptyState`, `ProgressBar`, `MetricCard`, `CapacityBar`, `HealthDonut`, `AlertCard` y `TimeSeriesChart` (nuevos esta sesión). `EmptyState` cubre vacío/no compatible/error de fuente a nivel de pantalla (es literalmente el componente que las pantallas usan para esos tres estados); `ProgressBar` cubre cargando; `MetricCard`/`CapacityBar`/`DiskCard` cubren dato ausente y `MetricCard` además dato obsoleto (marca de edad); `HealthDonut`/`AlertCard`/`TimeSeriesChart` cubren vacío propio (sin datos que dibujar). Los 19 restantes (`Card`, `Sidebar`, `Toolbar`, `AppShell`, `ConfirmDialog`, `DateRangePicker`, `FilterBar`, `Toast`, `RadioGroup`, `SegmentedControl`, `Select`, `Switch`, `TextField`, `DataRow`, `EventRow`, `StatusDot`, `StatusPill`, `CodeOutput`, `VirtualList`) son controles estructurales o de formato sin concepto propio de frescura de dato — renderizan lo que reciben, no deciden si un dato falta, ha caducado o no es compatible —, mismo criterio que ya documentaba el propio `DiskCard.browser.test.ts` para «cargando»/«error de fuente». Esta auditoría encontró y corrigió dos incumplimientos reales: `EmptyState.svelte` tenía cuatro literales de interfaz sin pasar por `t()` (`common.sourceError` es clave nueva en ambos diccionarios) y `AlertCard.svelte` declaraba `onselect` sin `= undefined`, incumpliendo la regla de callback opcional
- [X] T110 [P] Ampliar `e2e/ui/a11y.spec.ts` a toda pantalla nueva, por ambos temas. Ya cubría las 7 rutas reales (`/`, `/alerts`, `/disks/disk-0`, `/events`, `/tests`, `/reports`, `/settings`) desde que se fueron construyendo en las historias 1-8: cada pantalla nueva se añadió a `PANTALLAS` en el mismo commit que la creaba, no queda ninguna sin cubrir
- [X] T111 Comprobar el escalado al 125 %, 150 % y 200 % y la ventana mínima 1024 × 560 en todas las rutas de `src/routes/`, según la definición de terminado de `docs/ui-design.md` §8. Medido con Playwright (`e2e/ui/escalado.spec.ts`, 35 pruebas: 7 pantallas × 5 tamaños — 1024×560, 1280×720, y los tres equivalentes en píxeles CSS reales de escalar una ventana de 1024×560 al 125/150/200 %, `open-questions.md` J.34 explica por qué el zoom del navegador no sirve para esto). Encontró y corrigió un incumplimiento real de §4.0.bis: la `Sidebar` nunca colapsaba a 56 px por debajo de 1180 px, y eso truncaba texto de verdad ("Panel gene...", "No" en vez de "No disponible", el botón "Actualizar" cortado). Corregido con un marcador de inicial por sección como paso intermedio, autorizado explícitamente por el usuario a falta de iconos en el boceto aprobado (`open-questions.md` J.41). Las 35 pruebas y las capturas visuales confirman el arreglo en las cinco medidas; `Sidebar.browser.test.ts` cubre que el nombre accesible se conserva al colapsar
- [X] T112 [P] Verificar con un acento del sistema claro (`#ffb900`) en ambos temas que el contraste sigue cumpliendo AA, con la herramienta `tools/accent-check.py`. Reejecutado el barrido completo (paso 4, 262.144 colores del espacio sRGB, no solo la muestra): `#ffb900` pasa como fondo de botón (10.9:1) y como texto en ambos temas tras `accentOnSurface()` (la tabla de referencia del propio script lo incluye). Verificación global: 0 % de colores por debajo de AA tras el tratamiento, en los dos temas — I.4 ya lo daba por resuelto de una sesión anterior; esto confirma que sigue siéndolo con el código actual
- [ ] T113 Cerrar en `docs/open-questions.md` las cuestiones I.2, I.5, I.7 y la de umbrales de espacio, con el dato medido. **Parcial**: I.7 cerrada (dato ya medido en T074/J.26, la fila solo estaba desactualizada). Las otras tres siguen `ABIERTO` porque piden algo que esta sesión no puede producir sin autorización o hardware que no está disponible: I.2 (notificaciones toast elevadas) exige un instalador real empaquetado y probado; I.5 (cascada de `smartctl` tras RAID/USB) exige hardware físico variado; J.13 (umbrales de espacio de 1 GB/256 MB) es un valor de partida razonable, no medido, y medirlo de verdad exigiría observar el comportamiento con el historial creciendo en un volumen real durante tiempo
- [X] T114 [P] Actualizar `docs/roadmap.md` y regenerar el consolidado con `pnpm docs:build`. Fase 0 revisada bullet a bullet contra el estado real (la mayoría resuelta, con referencia a su sección de `open-questions.md`; solo I.2, I.5 y el contraste en Windows Server siguen `ABIERTO`); versiones 0.1-0.9 marcadas como implementadas; versión 1.0 con sus pendientes reales (T111, T115, release y firma). `pnpm docs:check` confirma que `historias.md` está al día (31 ficheros, 7.810 líneas)
