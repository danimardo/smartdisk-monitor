...
{#if open}
  ...
  >
    ...
    {#if action}
      <button
        class="rounded-pill px-3 py-1 text-xs font-semibold text-accent hover:bg-accent-soft"
        onclick={onaction}>{action}</button
      >
    {/if}
    <button
      class="grid size-6 shrink-0 place-items-center rounded-pill text-fg-faint hover:bg-glass-3"
      aria-label="Cerrar"
