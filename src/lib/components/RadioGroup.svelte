<script lang="ts">
  ...
  let {
    value = "",
    label = "",
    options = [] as { id: string; label: string; hint?: string }[],
    onchange
  } = $props();
</script>
  ...
  {#each options as opt (opt.id)}
    <button
      ...
      class="flex items-start gap-3 rounded-inner border p-3 text-left transition-all duration-base ease-sdm
        ...
        : 'border-hairline bg-glass-2 hover:bg-glass'}"
      onclick={() => onchange?.(opt.id)}
    >
      <span
        class="mt-0.5 grid size-4 shrink-0 place-items-center rounded-pill border {value === opt.id
          ? 'border-accent'
          : 'border-hairline'}"
      >
